...
public class HMCExceptionHandler {
	...
	/**
	 * Handles uncontrolled exceptions.
	 * @param exception an uncontrolled exception.
	 * @return a {@link ResponseEntity} of {@link HMCErrorResponse} with http internal
	 * server error status and an exception error message.
	 */
	@ExceptionHandler(Exception.class)
	@ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
	public HMCErrorResponse handleUncontrolledException(final Exception exception) {

		log.error(exception.getMessage(), exception);

		return new HMCErrorResponse(HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase());
	}
