...
class ItemLinksModelEntityConverterTest {
	...
	@Spy
	private ItemLinksModelEntityConverter testObj = Mappers.getMapper(ItemLinksModelEntityConverter.class);

	@Test
	void hyperwalletLocatorFromLinkTarget_shouldCreateHyperwalletItemLocator() {
		//@formatter:off
		ItemLinkEntity itemLinkEntity = ItemLinkEntity.builder()
		...
		HyperwalletItemLinkLocator result = testObj.hyperwalletLocatorFromLinkTarget(itemLinkEntity);
	...
	void from_shouldCreateEntityFromLocators() {
		HyperwalletItemLinkLocator hyperwalletItemLocator = new HyperwalletItemLinkLocator("H1",
				HyperwalletItemTypes.PAYMENT);
		MiraklItemLinkLocator miraklItemLocator = new MiraklItemLinkLocator("M1", MiraklItemTypes.SHOP);

		ItemLinkEntity itemLinkEntity = testObj.from(miraklItemLocator, hyperwalletItemLocator);
