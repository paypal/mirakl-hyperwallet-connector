...
`./gradlew build` cannot resolve `com.mirakl:mmp-sdk-operator:10.10.0` without
`PAYPAL_MIRAKL_SDK_USER` / `PAYPAL_MIRAKL_SDK_PASSWORD`: the Artifactory returns 403 and
the artifact is not on Maven Central. `hmc.lib-conventions` puts that dependency on every
library module, so **no module builds without them** on a clean environment.
...
from Maven Central, **if and only if** the artifacts are already in the Gradle cache from
a previous authenticated build.

To check which modules transitively depend on Mirakl:

```
./gradlew <module>:dependencies --configuration compileClasspath | grep mirakl
```

**Do not rely on `grep -rn "com.mirakl" src/` alone.** It only detects direct imports.
A module may have zero direct references but still fail to compile because of transitive
dependencies through `hmc-infrastructure`, which uses `com.mirakl` heavily.

If you must verify without credentials and with a populated cache:
...
2. Match the pinned versions rather than guessing: Spring Boot is in `buildSrc/build.gradle`,
   everything else follows from it (Boot 3.5.14 → Spring Framework 6.2.x).
3. Compile with an explicit file list and `-implicit:none`. Letting `-sourcepath` resolve
...
4. **Use JDK 21, not the project's Java 25 toolchain.** Lombok trails new JDK releases, and
   ...
   `cannot find symbol: method getStatus()` in code that is perfectly fine. `--release 21`
   is not enough — the running javac has to be a version Lombok supports.
...
it. For the serialization bug (batch job instance in JobDataMap with JDBC JobStore) that
meant putting the batch job instance back into the JobDataMap and confirming the
`NotSerializableException` the bug report showed — which is the difference between a test
that documents a fix and a test that would catch the bug coming back.
