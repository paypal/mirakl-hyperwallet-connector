...
public class QuartzBatchJobBean extends QuartzJobBean {
	...
	/**
	 * Sets the batch job to run. Invoked by Spring with the {@value #KEY_BATCH_JOB_BEAN}
	 * entry of the job data map.
	 * @param batchJob the fully qualified class name of the batch job bean.
	 */
	 ...
	 * identify or display the job do not have to load a class that may no longer exist:
	 * a persisted job outlives the code that defined it.
	...
	private BatchJob<? extends BatchJobContext, ? extends BatchJobItem<?>> resolveBatchJob()
		...
		try {
			...
			return (BatchJob<? extends BatchJobContext, ? extends BatchJobItem<?>>) beanFactory
				.getBean(batchJobClass);
		}
		catch (final ClassNotFoundException | LinkageError | BeansException e) {
			// The job outlived its class or its bean definition. Fail this execution
			// without refiring: retrying cannot make the class reappear.
			throw new JobExecutionException(
					"Unable to resolve batch job bean [" + batchJob + "]. The job is persisted but its class is no "
							+ "longer available in the application context.",
					e);
		}
	}
