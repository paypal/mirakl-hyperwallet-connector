...
public abstract class AbstractKYCUserStatusNotificationStrategy
	...
	protected final MiraklClient miraklOperatorClient;
	...
	protected AbstractKYCUserStatusNotificationStrategy(final MiraklClient miraklOperatorClient,
	...
	protected void updateShop(final KYCUserStatusNotificationBodyModel kycUserStatusNotificationBodyModel) {
		...
		if (Objects.nonNull(status)) {
			...
			try {
				...
				// If change staging is enabled, document deletion is omitted, since it
				// implies retrieving shops for each notification and then do a shop
				// update
				// for the deletion, and this would exceed Mirakl API rate limit.
				...
				final String errorMessage = MSG_ERROR_DETECTED.formatted(shopId, MiraklLoggingErrorsUtil.stringify(ex));
	...
	private void logErrorMessage(final MiraklUpdateShopWithErrors shopError) {
		...
		final String errorMessage = MSG_ERROR_DETECTED.formatted(shopId, miraklUpdateErrors);
