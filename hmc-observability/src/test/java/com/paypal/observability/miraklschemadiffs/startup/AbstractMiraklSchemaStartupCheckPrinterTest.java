...
class AbstractMiraklSchemaStartupCheckPrinterTest {
	...
	void print_shouldReturnReportEntryMessageAndSeverity() {
		final Map<String, Object> detailsMap = Map.of(MiraklDocSchemaStartupCheckProvider.STATUS_CHECK_DETAILS_DIFF_KEY,
		...
		final String[] result = testObj.print(startupCheckMock);

		assertThat(result).hasSize(2);
		assertThat(result[0]).isEqualTo("%s%n%s".formatted("MESSAGE-1", "Severity: BLOCKER"));
		assertThat(result[1]).isEqualTo("%s%n%s".formatted("MESSAGE-2", "Severity: RECOMMENDATION"));
	}
