...
public class MiraklFieldSchemaDiffEntrySeverityAssigner implements MiraklSchemaDiffEntrySeverityAssigner {
	...
	//@formatter:off
	private final Map<String, MiraklSchemaDiffReportSeverity> fieldNameToSeverityMap = Map.of(
	...
	// permissions.
	private final Map<MiraklFieldPermissions, Integer> permissionsLevelMap = Map.of(
	...
	public MiraklSchemaDiffReportSeverity getSeverityFor(final MiraklSchemaDiffEntry entry) {
		...
		if (MiraklSchemaDiffEntryType.ITEM_NOT_FOUND.equals(entry.getDiffType())) {
			...
			final MiraklSchemaDiffEntryIncorrectAttributeValue incorrectAttributeEntry = ((MiraklSchemaDiffEntryIncorrectAttributeValue) entry);
			final String fieldName = incorrectAttributeEntry.getAttributeName();
	...
	private MiraklSchemaDiffReportSeverity getPermissionsAttributeSeverity(
			final MiraklSchemaDiffEntryIncorrectAttributeValue diffEntry) {
		final MiraklFieldPermissions actual = ((MiraklField) diffEntry.getActual()).getPermissions();
		final MiraklFieldPermissions expected = ((MiraklField) diffEntry.getExpected()).getPermissions();
	...
	private MiraklSchemaDiffReportSeverity getGenericAttributeSeverity(
			final MiraklSchemaDiffEntryIncorrectAttributeValue diffEntry) {
		final String fieldName = diffEntry.getAttributeName();
