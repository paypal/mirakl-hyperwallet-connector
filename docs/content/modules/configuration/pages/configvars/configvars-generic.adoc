...
|Directory where the application stores its runtime data. When running via Paketo Buildpack (the default Docker image build), set this to `/home/cnb/data` — the process user `cnb` does not have write access to `/workspace`.
|`/home/cnb/data`
