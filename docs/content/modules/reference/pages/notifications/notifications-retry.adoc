...
All notifications are stored in the `NotificationEntity` table from the moment they arrive (see <<_storing_and_processing,Storing and processing>>). When processing a notification fails, the connector:
...
When all retry attempts for a notification are exhausted, the entry is set to `FAILED` and the connector sends an email alert to the operator. Failed entries are never retried again automatically, but they can be managed through the <<management-api,Failed Notifications Management API>>.
