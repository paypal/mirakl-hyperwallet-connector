...
class HyperWalletCreateBusinessStakeHolderServiceStrategyTest {
	...
	@Mock
	private BusinessStakeholderTokenUpdateService businessStakeholderTokenUpdateServiceMock;
	...
	void execute_shouldReturnCreatedHyperWalletBusinessStakeHolder() {
		...
		verify(businessStakeholderTokenUpdateServiceMock).updateBusinessStakeholderToken(CLIENT_ID,
	...
	void execute_shouldSendEmailNotificationHyperwalletExceptionIsThrown() {
		...
		verify(mailNotificationUtilMock).sendPlainTextEmail(
				...
				(ERROR_MESSAGE_PREFIX + "Business stakeholder not created for clientId [%s]%n%s").formatted(CLIENT_ID,
